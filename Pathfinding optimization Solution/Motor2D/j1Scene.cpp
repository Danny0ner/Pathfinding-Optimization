...
bool j1Scene::Update(float dt)
{
	...
	if (origin_selected == true)
	{
		or_select.assign("yes");
	}
	else if (origin_selected == false) {
		or_select.assign("no");
	}
	static char title[125];
	sprintf_s(title, 125, "PathfindingOptimized : %s Origin Selected : %s OptPath time: %f ms NormalPath time: %f ms", pathfinding.c_str() ,or_select.c_str(), lastoptimizedtime, lastnormaltime);
